...
export class ProfileController {
	static async getOne(session) {
		const profileData = await ProfileDao.getOne(session.data.userId).catch(
			(e) => {
		...
		profileData.userRoles.forEach((userRole) => {
			userRole.role.privileges.forEach((rp) => {
				...
				if (modulePermission[rp.module.name]) {
					...
					let newPermissionInt =
						parseInt(currentPermission, 2) | parseInt(newPermission, 2);
					let newPermissionStr = newPermissionInt.toString(2);

					modulePermission[rp.module.name] =
						convertToFourDigitBinary(newPermissionStr);
				} else {
	...
	static async updatePassword(data, session) {
		...
		// check old password
		const oldPasswordHash = createHash("sha512")
			.update(`${data.oldPassword}${process.env.SALT}`)
			.digest("hex");
		...
		user.password = createHash("sha512")
			.update(`${data.password}${process.env.SALT}`)
			.digest("hex");
